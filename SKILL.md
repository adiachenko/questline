...
Keep at most one unfinished Waypoint per Questline. Allocate a local integer only when the user explicitly asks to make specified work the Questline's next deliverable or accepts your proposal to do so; allocate a successor only after the current Waypoint is implemented and verified or explicitly abandoned. Authorization to perform the work, even when it is substantial or bounded, is not that choice. When work clearly merits a Waypoint, propose its boundary and ask the user whether to create it. Create it only after explicit agreement; if the user declines, proceed with the requested work without changing `.questline/`. Pad the identifier to at least three digits and never renumber or reuse identifiers. Replace an unaccepted proposal in place when its deliverable is discarded. If an accepted Waypoint is abandoned, record that outcome in its file so it remains historical intent.
